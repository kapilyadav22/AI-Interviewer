...
export const BrainTeaser = ({ teaser: propTeaser, loading: propLoading }) => {
  ...
  const { execute, loading: isLoadingAction } =
    useGeminiAction(setShowAuthWarning);
  const [isManualLoading, setIsManualLoading] = useState(false);

  const isLoading = propLoading || isLoadingAction || isManualLoading;
  ...
  const fetchTeaser = async (topic = selectedTopic) => {
    ...
    await execute(
      (service) =>
        service.getDailyTeaser(
          topic === "General" ? "Software Engineering" : topic,
        ),
      {
        onSuccess: (data) => {
          if (data) {
              ...
              return (
                ...
                >
                  <Icon
                    size={12}
                    className={cn(
                      selectedTopic === topic.id ? "fill-current" : "",
                    )}
                  />
                  {topic.label}
                </button>
          ...
          {!showAnswer ? (
            <Button
              onClick={() => {
                ...
                const currentMastery = parseInt(
                  localStorage.getItem("nextoffer_ai_logic_mastery") || "5",
                );
                localStorage.setItem(
                  "nextoffer_ai_logic_mastery",
                  JSON.stringify(Math.min(10, currentMastery + 0.5)),
                );
              }}
...
const CodeBlock = ({ inline, children, ...props }) => {
  ...
  if (inline) {
    return (
      <code
        className="px-1.5 py-0.5 bg-white/10 rounded text-amber-400 font-mono text-xs"
        {...props}
      >
        {children}
      </code>
