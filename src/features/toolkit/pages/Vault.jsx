...
export const Vault = () => {
  const [items, setItems] = useState(() => {
    try {
      const saved = localStorage.getItem("nextoffer_ai_vault");
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-2">
            ...
            <p className="text-slate-500 dark:text-slate-400 font-medium">
              Your library of AI-generated career assets.
            </p>
          </div>
        ...
        {filteredItems.length > 0 ? (
              ...
              {filteredItems.map((item) => (
                ...
                >
                  <Card className="h-full flex flex-col p-8 rounded-[2.5rem] border-slate-200 dark:border-slate-700 hover:border-amber-200 dark:hover:border-amber-800 transition-colors bg-white dark:bg-slate-800 shadow-xl hover:shadow-2xl overflow-hidden relative">
                    ...
                    <div className="flex items-center justify-between mb-6 relative z-10">
                      <div
                        className={cn(
                          ...
                          item.type === "Pitch"
                            ? "bg-indigo-600 shadow-indigo-100 dark:shadow-indigo-900/20"
                            : item.type === "Architecture"
                              ? "bg-blue-600 shadow-blue-100 dark:shadow-blue-900/20"
                              : item.type === "Referral"
                                ? "bg-emerald-600 shadow-emerald-100 dark:shadow-emerald-900/20"
                                : item.type === "Negotiation"
                                  ? "bg-amber-600 shadow-amber-100 dark:shadow-amber-900/20"
                                  : item.type === "Behavioral"
                                    ? "bg-purple-600 shadow-purple-100 dark:shadow-purple-900/20"
                                    : "bg-slate-600 shadow-slate-100 dark:shadow-slate-900/20",
                        )}
                      >
                        {item.type === "Pitch" ? (
                          <Sparkles size={24} />
                    ...
                    <h3 className="text-xl font-black text-slate-900 dark:text-white mb-2 truncate relative z-10">
                      {item.title}
                    </h3>
          ...
          <div className="py-24 flex flex-col items-center justify-center text-center space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-slate-500 dark:text-slate-400 font-medium">
                Start practicing to save your high-impact AI responses here.
              </p>
            </div>
            <Link to="/setup">
              <Button className="rounded-xl px-10 font-bold">
                PRACTICE NOW
              </Button>
            </Link>
          </div>
