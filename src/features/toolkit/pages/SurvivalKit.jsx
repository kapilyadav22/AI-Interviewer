...
import { 
    Shield, MessageCircle, Mail, Sparkles, 
    ChevronRight, ArrowLeft, RefreshCw, 
    Copy, Check, Bookmark, Target, Lightbulb,
    FileCheck, Send
} from 'lucide-react';
...
export const SurvivalKit = () => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const result = await gemini.generateSurvivalKit(formData.role, formData.company, formData.context);
                    ...
                    {gameState === 'input' ? (
                        ...
                        >
                            <div className="text-center mb-12">
                                ...
                                <h1 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white mb-4 tracking-tight uppercase">Post-Interview Game</h1>
                                <p className="text-slate-500 dark:text-slate-400 text-lg font-medium leading-relaxed">
                                    Win the post-interview game. Generate high-impact questions to ask and professional thank-you notes.
                                </p>
                            </div>
                            ...
                            <Card className="p-8 md:p-12 rounded-[2.5rem] border-slate-200 dark:border-slate-800 shadow-2xl space-y-10 dark:bg-slate-800">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-3 gap-4">
                                        {['First Round', 'Final Round', 'Executive Review'].map((stage) => (
                                            <button
                                                key={stage}
                                                onClick={() => setFormData({...formData, context: stage})}
                                                className={cn(
                                ...
                                >
                                    {isGenerating ? (
                                        <><RefreshCw size={24} className="animate-spin" /> CRAFTING YOUR MISSION... </>
                                    ) : (
                        ...
                        >
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                                {/* Left Column: Reverse Questions */}
                                <div className="space-y-8">
                                    <div className="flex items-center gap-3 mb-2">
                                        <div className="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-600">
                                            <Target size={20} />
                                        </div>
                                        <h2 className="text-2xl font-black text-slate-900 tracking-tight uppercase">Reverse Interviewing</h2>
                                    </div>
                                    {data.reverseQuestions.map((q, i) => (
                                        <Card key={i} className="p-6 rounded-2xl border-slate-100 dark:border-slate-700 shadow-lg hover:border-indigo-200 dark:hover:border-indigo-800 transition-colors space-y-4 dark:bg-slate-800">
                                            <h3 className="font-black text-slate-800 dark:text-white text-lg leading-tight">"{q.question}"</h3>
                                            ...
                                            >
                                                {copiedId === `q-${i}` ? <Check size={16} /> : <Copy size={16} />}
                                ...
                                {/* Right Column: Thank You AI & Talking Points */}
                                <div className="space-y-10">
                                    <div className="space-y-6">
                                        <div className="flex items-center gap-3 mb-2">
                                            ...
                                            <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase">Thank-You Note AI</h2>
                                        </div>
                                        <Card className="p-8 rounded-[2.5rem] border-slate-200 dark:border-slate-700 shadow-xl space-y-6 relative overflow-hidden group dark:bg-slate-800">
                                            ...
                                            <div className="flex gap-3 relative z-10">
                                                <Button 
                                                    ...
                                                    onClick={() => copyToClipboard(data.thankYouDraft, 'thankyou')}
                                                >
                                                ...
                                                <Button 
                                                    ...
                                                    onClick={() => saveToVault('Post-Interview', `Thank You for ${formData.company}`, data.thankYouDraft)}
                                                >
                                                    <Bookmark size={18} />
                                                </Button>
                                            </div>
                                    ...
                                    <div className="space-y-6">
                                        <div className="flex items-center gap-3 mb-2">
                                            ...
                                            <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase">Talking Points</h2>
                                        </div>
                                        <div className="space-y-4">
                                            {data.talkingPoints.map((point, i) => (
                                                <div key={i} className="flex items-center gap-4 bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-sm">
                                                    ...
                                                    <p className="text-slate-700 dark:text-slate-300 font-bold">{point}</p>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                            ...
                            <div className="p-10 bg-slate-900 rounded-[3rem] text-white flex items-center justify-between shadow-2xl relative overflow-hidden group">
                                ...
                                <div className="relative z-10 space-y-3">
                                    <h4 className="text-3xl font-black uppercase tracking-tight italic">Fortune favors the prepared.</h4>
                                    <p className="text-slate-400 font-medium max-w-lg">
                                        These questions and notes are strategically designed to show high intent and peak professional courtesy.
                                    </p>
                                </div>
                                <Button 
                                    onClick={handleGenerate}
