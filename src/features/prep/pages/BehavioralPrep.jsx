...
import { 
    ...
    Mic, MicOff, Send, ArrowLeft, Lightbulb,
    Target, Zap, Sparkles, ChevronRight, Bookmark
} from 'lucide-react';
...
export const BehavioralPrep = () => {
    ...
    const [gameState, setGameState] = useState('selection'); // selection, practicing, feedback
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            const result = await gemini.analyzeStarResponse(selectedQuestion.question, responseText);
            setAnalysis(result);
            setGameState('feedback');
        } catch (error) {
            // DEBUG: console.error("Analysis failed:", error);
            showToast("Failed to analyze response. Please check your API key.", "error");
        } finally {
                    ...
                    {gameState === 'selection' && (
                        ...
                        >
                            <div className="text-center mb-12">
                                ...
                                <h1 className="text-4xl font-black text-slate-900 dark:text-white mb-3 tracking-tight">STAR METHOD ANALYZER</h1>
                                <p className="text-slate-500 dark:text-slate-400 max-w-2xl mx-auto font-medium">
                                    Behavioral interviews are won through structure. Practice common questions and get real-time AI feedback on your <span className="text-indigo-600 dark:text-indigo-400 font-bold">Situation, Task, Action, and Result</span>.
                                </p>
                            </div>
                    ...
                    {gameState === 'practicing' && (
                        ...
                        >
                            ...
                            <div className="bg-indigo-600 rounded-3xl p-8 mb-8 text-white shadow-xl shadow-indigo-200 relative overflow-hidden">
                                ...
                                <span className="text-indigo-200 text-[10px] uppercase font-black tracking-widest mb-2 block">Behavioral Challenge</span>
                            ...
                            <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm p-6 space-y-4">
                                <div className="flex items-center justify-between mb-2">
                                    <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Your Response</span>
                                    <button 
                                ...
                                >
                                    {isAnalyzing ? (
                                        <><RefreshCw className="w-5 h-5 mr-3 animate-spin" /> Analyzing STAR Performance...</>
                                    ) : (
                        ...
                        >
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex gap-4">
                                    <ScoreBadge label="Structure" score={analysis.scores.structure} color="indigo" />
                                    <ScoreBadge label="Impact" score={analysis.scores.impact} color="emerald" />
                                    <ScoreBadge label="Clarity" score={analysis.scores.clarity} color="amber" />
                                </div>
                            </div>
                                ...
                                <div className="space-y-6">
                                    <Card className="border-indigo-100 dark:border-indigo-900/50 bg-indigo-50/30 dark:bg-indigo-900/10 overflow-hidden">
                                        <CardHeader className="bg-white dark:bg-slate-800 border-b border-indigo-50 dark:border-indigo-900/30">
                                            <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400">
                                                <Target size={20} />
                                                <h3 className="font-black uppercase tracking-widest text-sm">STAR Breakdown</h3>
                                            </div>
                                        </CardHeader>
                                        <CardBody className="p-0">
                                            <StarStep label="S" title="Situation" content={analysis.star_breakdown.situation} />
                                            <StarStep label="T" title="Task" content={analysis.star_breakdown.task} />
                                            <StarStep label="A" title="Action" content={analysis.star_breakdown.action} />
                                            <StarStep label="R" title="Result" content={analysis.star_breakdown.result} />
                                        </CardBody>
                                    </Card>
                                    ...
                                    <div className="bg-slate-900 rounded-3xl p-8 text-white shadow-2xl">
                                        <div className="flex items-center gap-2 mb-4 text-emerald-400">
                                            <Sparkles size={20} />
                                            <h3 className="font-black uppercase tracking-widest text-sm">Coach's Feedback</h3>
                                        </div>
                                    ...
                                    <Card className="border-emerald-100 dark:border-emerald-900/30">
                                        <CardHeader className="bg-emerald-50/50 dark:bg-emerald-900/10 border-b border-emerald-100 dark:border-emerald-900/30 flex justify-between items-center">
                                            <div className="flex items-center gap-2 text-emerald-700 dark:text-emerald-400">
                                                <Zap size={20} />
                                                <h3 className="font-black uppercase tracking-widest text-sm">Improved Version</h3>
                                            </div>
                                            <Button variant="ghost" size="sm" className="text-emerald-700 dark:text-emerald-400 font-bold" onClick={() => {
...
const ScoreBadge = ({ label, score, color }) => {
    ...
    return (
        <div className={cn("px-4 py-3 rounded-2xl border flex flex-col items-center min-w-[100px]", colors[color])}>
            <span className="text-[10px] uppercase font-black tracking-widest mb-1 opacity-70">{label}</span>
