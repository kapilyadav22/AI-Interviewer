...
import { getInterviews, getRecordingUrl, deleteInterview } from '../../../shared/services/supabase';
import { Calendar, Clock, PlayCircle, FileText, ChevronDown, ChevronUp, Trash } from 'lucide-react';
...
export function History() {
    ...
    if (!user) {
        return <AuthRequired title="Interview History" message="Please sign in to view your past interviews and feedback." />;
    }

    return (
        <Layout>
            <div className="max-w-4xl mx-auto space-y-8">
                <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Interview History</h1>
                ...
                {interviews.length === 0 ? (
                    <div className="text-center py-12 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
                        <p className="text-slate-500 dark:text-slate-400">No interviews found yet.</p>
                    </div>
                ) : (
                    <div className="space-y-4">
                        {interviews.map((interview) => (
                            <div key={interview.id} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                                <div
                                ...
                                >
                                    ...
                                    {expandedId === interview.id ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
                                    {/* Delete button */}
                                    <button
                                        onClick={(e) => {
                                            e.stopPropagation();
