...
export const PitchCoach = () => {
  ...
  const handleAnalyze = async () => {
    ...
    await execute((service) => service.analyzePitch(pitch, role), {
      onSuccess: (result) => {
        if (result) {
          setAnalysis(result);
            ...
            >
              <div className="text-center mb-12">
                ...
                <p className="text-slate-500 dark:text-slate-400 text-lg font-medium leading-relaxed">
                  Master the first 2 minutes. We'll analyze your "Tell me about
                  yourself" opener and turn it into a high-impact "Platinum
                  Version."
                </p>
              </div>
              ...
              <Card className="p-8 md:p-10 rounded-[2.5rem] border-slate-200 dark:border-slate-700 shadow-2xl space-y-8 dark:bg-slate-800">
                ...
                >
                  {isAnalyzing ? (
                    <>
                      <RefreshCw size={24} className="animate-spin" /> ANALYZING
                      PITCH...
                    </>
                  ) : (
                    <>
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <ScoreCard
                  icon={<Target size={20} />}
                  label="Impact"
                  val={analysis.scores.impact}
                  color="indigo"
                />
                <ScoreCard
                  icon={<Clock size={20} />}
                  label="Conciseness"
                  val={analysis.scores.conciseness}
                  color="blue"
                />
                <ScoreCard
                  icon={<Zap size={20} />}
                  label="Hook Factor"
                  val={analysis.scores.hook}
                  color="amber"
                />
              </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <ul className="space-y-3">
                        {analysis.feedback.positives.map((p, i) => (
                          <li
                            key={i}
                            className="flex gap-3 text-slate-600 dark:text-slate-300 font-medium"
                          >
...
const ScoreCard = ({ icon, label, val, color }) => {
  ...
  return (
    <Card
      className={cn(
        "p-8 text-center space-y-4 rounded-[2rem] border-b-8 shadow-sm dark:bg-slate-800",
        colors[color],
      )}
    >
      ...
      <div className="text-5xl font-black">
        {val}
        <span className="text-lg opacity-40">/10</span>
      </div>
