...
import { 
    Newspaper, Calendar, Clock, User, 
    ArrowRight, ArrowLeft, Share2, 
    Bookmark, Search, Tag, X, ChevronRight,
                        ...
                        >
                            ...
                            <div className="text-center max-w-3xl mx-auto space-y-4">
                                ...
                                <h1 className="text-5xl md:text-6xl font-black text-slate-900 dark:text-white tracking-tight leading-none">
                                    THE MOCKMATE <span className="text-primary-600 font-style-italic">RESOURCES</span>
                                </h1>
                                <p className="text-slate-500 dark:text-slate-400 text-lg font-medium leading-relaxed">
                                    Stay ahead in your career loop with expert interview tips, technical deep-dives, and behavioral blueprints.
                                </p>
                            </div>
                                    ...
                                    >
                                        ...
                                        <div className="p-8 flex-1 flex flex-col">
                                            <div className="flex items-center gap-4 text-slate-400 text-[10px] font-black uppercase tracking-widest mb-4">
                                                <span className="flex items-center gap-1.5 font-bold"><Calendar size={14} /> {post.date}</span>
                                                <span className="flex items-center gap-1.5 font-bold"><Clock size={14} /> {post.readTime}</span>
                                            </div>
                            ...
                            {filteredPosts.length === 0 && (
                                <div className="text-center py-32 bg-slate-50 dark:bg-slate-800 rounded-[3rem] border-2 border-dashed border-slate-200 dark:border-slate-700">
                                    ...
                                    <p className="text-xl font-bold text-slate-400 dark:text-slate-500">No articles matched your search.</p>
                                </div>
                            )}
                        </motion.div>
                    ) : (
                        ...
                        >
                            ...
                            <div className="mb-12">
                                ...
                                <div className="flex items-center justify-between py-8 border-y border-slate-100">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="font-black text-slate-900 dark:text-white uppercase text-xs tracking-widest">{selectedPost.author}</p>
                                            <p className="text-sm text-slate-400 font-bold">{selectedPost.date} • {selectedPost.readTime}</p>
                                        </div>
                                    </div>
                            ...
                            <div className="mt-20 p-12 bg-slate-900 rounded-[3rem] text-white flex flex-col md:flex-row items-center justify-between gap-8">
                                <div className="space-y-2">
                                    <h3 className="text-2xl font-black tracking-tight">Enjoyed this article?</h3>
                                    <p className="text-slate-400 font-medium">Get the latest interview blueprints delivered to your inbox.</p>
                                </div>
                                <div className="flex gap-4 w-full md:w-auto">
                                    ...
                                    />
                                    <Button className="font-bold rounded-2xl whitespace-nowrap">Subscribe</Button>
                                </div>
                            </div>
                        </motion.div>
                    )}
