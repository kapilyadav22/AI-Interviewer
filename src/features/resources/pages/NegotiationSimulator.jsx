...
export const NegotiationSimulator = () => {
  ...
  const handleStart = async () => {
    ...
    await execute(
      async (service) => {
        ...
        await service.startNegotiationChat(offerData);

        // Get initial greeting from Alex
        setIsTyping(true);
        setGameState("chatting");
        const result = await service.chat.sendMessage(
          "Hello Alex, I'd like to discuss the offer.",
        );
      ...
      },
      {
        onError: () => {
          showToast(
            "Failed to start negotiation. Check your API key.",
            "error",
          );
        },
      },
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      showToast(
        "Connection lost. Please check your API key and try again.",
        "error",
      );
    } finally {
      setIsTyping(false);
    ...
    await execute(
      async (service) => {
        const history = messages
          .map(
            (m) => `${m.role === "ai" ? "Recruiter" : "Candidate"}: ${m.text}`,
          )
          .join("\n");
        const result = await service.analyzeNegotiation(history);
        setAnalysis(result);
        ...
        localStorage.setItem(
          "nextoffer_ai_negotiation_outcomes",
      ...
      },
      {
        onError: () => {
          ...
          showToast(
            "AI Analysis failed. You can still save your raw transcript.",
            "error",
          );
        },
      },
            ...
            >
              <div className="text-center mb-12">
                ...
                <p className="text-slate-500 dark:text-slate-400 text-lg font-medium leading-relaxed">
                  Practice the most high-stakes conversation of your career.
                  Roleplay with an AI recruiter to maximize your compensation.
                </p>
              </div>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 shadow-xl overflow-hidden">
                <div className="p-8 md:p-12 space-y-8">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setOfferData({ ...offerData, role: e.target.value })
                        }
                      />
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setOfferData({ ...offerData, base: e.target.value })
                          }
                        />
                      </div>
                      <div className="space-y-2">
                  ...
                  <p className="text-center text-slate-400 text-xs font-medium">
                    Alex, your Hiring Manager, is ready to discuss these terms
                    with you.
                  </p>
                </div>
              </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-black text-slate-900 dark:text-white uppercase tracking-tight">
                      {offerData.company} • Hiring Discussion
                    </h3>
                ...
                <div className="flex-1 p-8 overflow-y-auto space-y-8 bg-slate-50/50 dark:bg-slate-900/50">
                  ...
                  {isTyping && (
                    <div className="flex items-center gap-3 text-slate-400 italic text-sm px-14">
                      <RefreshCw size={16} className="animate-spin" /> Alex is
                      thinking...
                    </div>
                  )}
                  {isAnalyzing && (
                    <div className="flex flex-col items-center justify-center p-12 bg-white/50 dark:bg-slate-800/50 rounded-[2rem] border-2 border-dashed border-emerald-200 dark:border-emerald-800 animate-pulse">
                      ...
                      <p className="font-black text-slate-900 dark:text-white uppercase tracking-widest">
                        Generating Your Report...
                      </p>
                      <p className="text-xs text-slate-400 font-bold mt-2">
                        Alex is reviewing your leverage and tone.
                      </p>
                    </div>
                  )}
                  {analysisError && (
                    <div className="flex flex-col items-center justify-center p-8 bg-red-50 dark:bg-red-900/10 rounded-[2rem] border border-red-100 dark:border-red-900/30 text-center">
                      ...
                      <h4 className="font-black text-red-900 dark:text-red-400 uppercase tracking-widest mb-2">
                        Analysis Failed
                      </h4>
                      <p className="text-sm text-red-600 dark:text-red-300 font-bold mb-6 max-w-sm">
                        We couldn't generate the AI report, likely due to an API
                        key issue. Save your transcript below to keep your work.
                      </p>
                      <div className="flex gap-4">
                        <Button
                          onClick={handleSaveFallback}
                          className="bg-slate-900 hover:bg-black text-white px-8 font-black"
                        >
          ...
          {gameState === "feedback" && analysis && (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <ScoreCard
                  label="Outcome"
                  val={analysis.outcome}
                  color="emerald"
                  icon={<Award size={20} />}
                />
                <ScoreCard
                  label="Negotiation Tone"
                  val={`${analysis.scores.tone}/10`}
                  color="blue"
                  icon={<MessageSquare size={20} />}
                />
                <ScoreCard
                  label="Leverage Usage"
                  val={`${analysis.scores.leverage_usage}/10`}
                  color="amber"
                  icon={<Zap size={20} />}
                />
              </div>
              ...
              <Card className="bg-emerald-50 dark:bg-emerald-900/20 border-emerald-100 dark:border-emerald-800 p-8 rounded-[2.5rem]">
                <h3 className="text-emerald-800 dark:text-emerald-300 font-black uppercase text-xs tracking-widest mb-6">
                  Expert Negotiation Tips for You
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {analysis.negotiation_tips.map((tip, i) => (
                    <div
                      key={i}
                      className="flex items-start gap-3 bg-white/50 dark:bg-slate-800/50 p-4 rounded-2xl border border-emerald-100/50 dark:border-emerald-800/50"
                    >
                      <div className="w-8 h-8 rounded-lg bg-emerald-600 text-white flex items-center justify-center flex-shrink-0 font-bold">
                        {i + 1}
                      </div>
                      <p className="text-emerald-900 dark:text-emerald-100 font-bold text-sm leading-relaxed">
                        {tip}
                      </p>
                    </div>
                  ))}
...
const ScoreCard = ({ label, val, color, icon }) => {
  ...
  return (
    <Card
      className={cn(
        "p-6 text-center space-y-2 border-b-8 shadow-sm",
        colors[color],
      )}
    >
